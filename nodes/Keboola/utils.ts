import { URL } from 'url';
import { LoggerProxy, UnexpectedError } from 'n8n-workflow';
...
export function detectCloudProvider(url: string): Platform {
	...
	if (url.startsWith('azure://') || url.includes('.blob.core.windows.net')) {
		return 'azure';
	}
...
export function extractAwsRegion(url: string): string {
	try {
		...
		LoggerProxy.warn('Failed to extract AWS region from URL', { url, error });
		return 'us-east-1';
	}
}
